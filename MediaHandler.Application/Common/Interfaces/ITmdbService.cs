...
public interface ITmdbService
{
    ...
    Task<TmdbIdLookupResult?> GetMovieByIdAsync(int tmdbId, string language = "en-US", CancellationToken cancellationToken = default);

    /// <summary>Looks up a TV show by its TMDB id. Returns null when the id does not exist.</summary>
    Task<TmdbIdLookupResult?> GetTvShowByIdAsync(int tmdbId, string language = "en-US", CancellationToken cancellationToken = default);
