...
public class IncrementalScanIdempotencyTests : ScannerIntegrationTestBase
{
    ...
    public async Task Sc005_IncrementalScan_UnchangedAndFast()
    {
        ...
        await WaitForScanCompletion(fullHandle.ScanRunId, 60);
        ...
        var incrHandle = await coordinator.StartAsync(
            ...
            TestContext.Current.CancellationToken);
        await WaitForScanCompletion(incrHandle.ScanRunId, 30);
        ...
        if (!isCi && fullDuration > 1.0)
        {
            ...
            ratio.Should().BeLessThan(0.65,
                $"SC-005 requires incremental < 65% of full ({incrDuration:F2}s / {fullDuration:F2}s = {ratio:P0})");
        }
    }
